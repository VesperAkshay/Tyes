pub const BASE: u32 = 1626;

/// Returns the number of words required to encode input data of length bytes.
pub fn words_required(length: usize) -> usize {
    ((length + 1) * 3) / 4
}

/// Encodes src into mnemonic words which are appended to dst.
pub fn encode_word_list(dst: &mut Vec<String>, mut src: &[u8]) {
    while src.len() >= 4 {
        let mut x = src[0] as u32;
        x |= (src[1] as u32) << 8;
        x |= (src[2] as u32) << 16;
        x |= (src[3] as u32) << 24;
        src = &src[4..];

        let i0 = (x % BASE) as usize;
        let i1 = ((x / BASE) % BASE) as usize;
        let i2 = ((x / BASE / BASE) % BASE) as usize;

        dst.push(WORD_LIST[i0].to_string());
        dst.push(WORD_LIST[i1].to_string());
        dst.push(WORD_LIST[i2].to_string());
    }
    ...
    if !src.is_empty() {
        ...
        dst.push(WORD_LIST[i].to_string());

        if src.len() >= 2 {
            let i = ((x / BASE) % BASE) as usize;
            dst.push(WORD_LIST[i].to_string());
        }

        if src.len() == 3 {
            let i = (BASE + ((x / BASE / BASE) % 7)) as usize;
            dst.push(WORD_LIST[i].to_string());
        }
    }
}
...
mod tests {
    ...
    fn test_words_required() {
        ...
        assert_eq!(words_required(2), 2);
        assert_eq!(words_required(3), 3);
        assert_eq!(words_required(4), 3);
        assert_eq!(words_required(5), 4);
        assert_eq!(words_required(8), 6);
        assert_eq!(words_required(12), 9);
    }

    #[test]
    fn test_encode_word_list() {
        let mut dst = Vec::new();
        encode_word_list(&mut dst, &[]);
        assert_eq!(dst.len(), 0);

        let mut dst = Vec::new();
        encode_word_list(&mut dst, &[0x01]);
        assert_eq!(dst.len(), 1);

        let mut dst = Vec::new();
        encode_word_list(&mut dst, &[0x01, 0x02]);
        assert_eq!(dst.len(), 2);
        
        let mut dst = Vec::new();
        encode_word_list(&mut dst, &[0x01, 0x02, 0x03]);
        assert_eq!(dst.len(), 3);

        let mut dst = Vec::new();
        encode_word_list(&mut dst, &[0x01, 0x02, 0x03, 0x04]);
        assert_eq!(dst.len(), 3);
        
        let mut dst = Vec::new();
        encode_word_list(&mut dst, &[0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]);
        assert_eq!(dst.len(), 6);
    }
}
